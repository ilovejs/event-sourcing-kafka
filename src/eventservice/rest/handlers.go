...
func (eh *eventServiceHandler) findEventHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		_, _ = fmt.Fprint(w, `No search criteria found, you can either search by id via /id/4 
	...
	if !ok {
		_, _ = fmt.Fprint(w, `No search keys found, you can either search by id via /id/4 to search by name via /name/coldplayconcert`)
		return
	}
	...
	if err != nil {
		w.WriteHeader(404)
		_, _ = fmt.Fprintf(w, "Error occured %s", err)
		return
	}
